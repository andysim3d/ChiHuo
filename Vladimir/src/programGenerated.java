...
public class programGenerated {
	...
	public void mfTableGenerator() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (exist) {
					al.get(Pos)._0_quant = update(al.get(Pos)._0_quant,
							+rs.getInt("quant"));
					continue;
				} else {
				...
				for (int i = 0; i < al.size(); i++) {
					if (rs.getString("cust").equals(al.get(i).cust)) {
						if (rs.getString("prod").equals(al.get(i).prod)) {
							if (rs.getInt("quant") < al.get(i)._0_quant
									.getAvg()) {
	...
	public void print() {
		System.out.println(outPutFormat.outPutFormats("cust", 8) + " "
				+ outPutFormat.outPutFormats("prod", 8) + " "
				+ outPutFormat.outPutFormats("1_count_ALL", 13) + " "
				+ outPutFormat.outPutFormats("2_count_ALL", 13) + " " + " ");
		for (mfTableBean mfb : al) {
			System.out.println("" + outPutFormat.outPutFormats(mfb.cust, 8)
					+ " " + outPutFormat.outPutFormats(mfb.prod, 8) + " "
					+ outPutFormat.outPutFormats(mfb._1_ALL.Count, 13) + " "
					+ outPutFormat.outPutFormats(mfb._2_ALL.Count, 13) + " ");
		}
	}
}
